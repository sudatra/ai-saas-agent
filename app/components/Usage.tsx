...
const Usage = ({ featureFlag, title }: { featureFlag: FeatureFlag, title: string }) => {
      ...
      <div className='bg-white rounded-2xl shadow-sm border border-red-100 p-6'>
        <div className='flex justify-between items-center mb-4'>
          ...
          <div className='px-4 py-2 bg-red-50 rounded-lg'>
            ...
            <span className='text-red-700 font-medium'>{featureAllocation}</span>
          </div>
        </div>
        ...
        <div className='relative'>
          ...
          <p className='text-sm text-red-600 mt-2'>
            You have used up all available tokens. Please upgrade your plan to
            using this feature
          </p>
        </div>
      </div>
  ...
  if(!isFeatureEnabled) {
    return (
      <div className='bg-white rounded-2xl shadow-sm border border-red-100 p-6 opacity-50'>
        <div className='flex justify-between items-center mb-4'>
          <h2 className='text-xl font-semibold text-gray-800'>{title}</h2>
          <div className='px-4 py-2 bg-red-50 rounded-lg'>
